...
object rewrites {
  ...
  @annotation.implicitNotFound(msg = "Cannot rewrite morphism ${M} to ${OM} using rewrite strategy ${RS}")
  case class RewriteFrom[
    ...
    def apply(morph: InMorph): OutMorph = rwrt(morph)
  }

  trait AnyRewriteStrategy { rs =>

    implicit def id_rewrite[M <: AnyGraphMorphism]:
        RewriteFrom[this.type, M, M] =
    new RewriteFrom[this.type, M, M]( identity[M] )

    type rewriteTo[M0 <: AnyGraphMorphism, OM0 <: M0#In --> M0#Out] = RewriteFrom[rs.type, M0, OM0]
    def rewriteTo[M0 <: AnyGraphMorphism, OM0 <: M0#In --> M0#Out]
      (rwrt: M0 => OM0) = 
      new rewriteTo(rwrt)
  }   
